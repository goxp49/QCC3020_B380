...
{
    //check dut mode
    if((appUserIsDutState() == TRUE)&&(status == pairingHandsetTimeout))
    {
        return;
    }


    //When the phone rejects, re-enter the pairing state
    if(appConfigIsLeft() && !appDeviceIsHandsetAnyProfileConnected() && (status == pairingHandsetAuthenticationFailed))
    {
        DEBUG_LOG("###PAIRING_STATE_HANDSET_DISCOVERABLE#####");
        ...
        case PAIRING_STATE_PEER_PENDING_AUTHENTICATION:
        {
            ...
            if (cfm->status == auth_status_success)
            {
                ...
                if(cfm->bonded)
                {
                    ...
                    //add to display idle led when first peer paired
                    appUiIdleActive();
                }
