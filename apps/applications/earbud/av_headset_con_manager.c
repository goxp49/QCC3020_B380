...
static void appConManagerHandleClDmAclOpenedIndication(const CL_DM_ACL_OPENED_IND_T *ind)
{
	...
	DEBUG_LOG("system state, %x, allow connected %d",appGetState(),appGetConManager()->handset_connect_allowed );
    if (ind->status == hci_success)
    {
        const bool is_local = !!(~ind->flags & DM_ACL_FLAG_INCOMING);
        const bool is_ble = !!(ind->flags & DM_ACL_FLAG_ULP);

        //Is peer request?Reject request when first handset pairing,prevent errors
        //1.No peer's records?
        if(!appDeviceGetPeerBdAddr(NULL) && !appDeviceIsHandset(&ind->bd_addr.addr))
        {
            //2.In the handset pairing?
            if(appGetState() == APP_STATE_HANDSET_PAIRING)
            {
                //3.device id verify
                if(ind->dev_class == (AUDIO_MAJOR_SERV_CLASS | RENDER_MAJOR_SERV_CLASS |AV_MAJOR_DEVICE_CLASS | HEADSET_MINOR_DEVICE_CLASS))
                {
                    appConManagerSendCloseAclRequest(&ind->bd_addr.addr,TRUE);
                    return;
                }
            }
        }
        ...
        else 
        {
            ...
            if (handset && theConMgr->handset_connect_allowed)
            {
                ...
                else
                {
                    ...
                    authorise = FALSE;
#else
                    //fix linkback fail and new handset connect fail after both of  ear reboot 
                    DEBUG_LOG("appConManagerHandleClSmAuthoriseIndication, ALLOW as a different handset (bredr)");
                    authorise = TRUE;
#endif
                }
            }
...
bool appConManagerSetDeviceStateDisconnected(void)
{
	bdaddr addr;

	if(appUiGetStoreAddr(&addr,PSKEY_CURRENT_USE_ADDR))
	{
		conManagerDevice *device = appConManagerFindDeviceFromBdAddr(&addr);
		appConManagerSetDeviceState(device, ACL_DISCONNECTED);
		BdaddrSetZero(&device->addr);
