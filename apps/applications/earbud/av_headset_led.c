...
static void appLedUpdate(ledTaskData *theLed)
{
    ...
    switch (nleds)
    {
        ...
        case 3:  
		{
			if((led_state & 0x04) && theLed->led_dimState)/*0~0xfff*/
	       	{
             	LedConfigure(2, LED_DUTY_CYCLE, theLed->led_dimState);
                LedConfigure(2, LED_PERIOD, 80);
	       	}
	       	LedConfigure(2, LED_ENABLE, led_state & 0x04 ? 1 : 0);
        }
    	case 2:  
		{
			if((led_state & 0x02)&&theLed->led_dimState)
	        {
				LedConfigure(1, LED_DUTY_CYCLE, theLed->led_dimState);
				LedConfigure(1, LED_PERIOD, 80);
	        }
	        LedConfigure(1, LED_ENABLE, led_state & 0x02 ? 1 : 0);
    	}
    	case 1: 
		{
			if((led_state & 0x01)&&theLed->led_dimState)
	    	{
                LedConfigure(0, LED_DUTY_CYCLE, theLed->led_dimState);
                LedConfigure(0, LED_PERIOD, 80);
	    	}
			LedConfigure(0, LED_ENABLE, led_state & 0x01 ? 1 : 0);
    	}
      #else
    ...
    for (;;)
    {
        ...
        switch (pattern->code)
        {
			...
			case LED_PATTERN_DIM_ON:
			{
                ...
                /* Turn on LED */
                theLed->led_state |= pattern->data>>8;
				dimTime = pattern->data & 0xff;
				if(theLed->led_dimCounter < dimTime)
				{
                    theLed->led_dimCounter++;
					theLed->led_dimState = (4096/dimTime)*theLed->led_dimCounter;
					MessageSendLater(&theLed->task, LED_INTERNAL_UPDATE, 0,50);
					return update_leds;
				}
				else
				{
					theLed->led_dimCounter = 0;
					theLed->led_dimState = 4096;
					/* Move to next instruction */
		        	stack->position++;
				}			  	            
			}
			...
			case LED_PATTERN_DIM_OFF:
			{
				...
				{
                    theLed->led_dimCounter++;
					theLed->led_dimState = (4096/dimTime)*(dimTime-theLed->led_dimCounter);
					MessageSendLater(&theLed->task, LED_INTERNAL_UPDATE, 0,20);
					return update_leds;
				}
				else
				{
					theLed->led_dimCounter = 0;
					theLed->led_dimState = 0;
					/* Move to next instruction */
		        	stack->position++;
				}
			}
