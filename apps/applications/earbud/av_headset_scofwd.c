...
extern uint16 can_start_a2dp_stream;
...
static void ProcessOTAControlMessage(uint8 ota_msg_id, const uint8* payload, int payload_size)
{
    ...
    switch (ota_msg_id)
    {
		...
		/*! user define OTA message*/
		case SFWD_OTA_MSG_USER_POWER_OFF:
			{
				DEBUG_LOG("SFWD_OTA_MSG_USER_POWER_OFF");
				if(appUserIsHeadsetPowerOn())
				{
					appPowerOffRequest();
					appUserDisablePowerOff();
...
#ifdef LINK_BACK_LED_SYNC
        case SFWD_OTA_MSG_SYNC_LED_LINKBACK:
            DEBUG_LOG("SFWD_OTA_MSG_SYNC_LED_LINKBACK");
        ...
        case SFWD_OTA_MSG_SYNC_LED_HANDSET_PAIRING_SUCCESS:
            DEBUG_LOG("SFWD_OTA_MSG_SYNC_LED_HANDSET_PAIRING_SUCCESS");
            if(appUserIsHeadsetPowerOn() && appChargerIsDisconnected())
            {
                appLedSetPattern(app_led_pattern_pair_success, LED_PRI_MED); \
            }
            break;
        ...
        case SFWD_OTA_MSG_SYNC_LED_HANDSET_CONNECTED_IDLE:
            {
                DEBUG_LOG("SFWD_OTA_MSG_SYNC_LED_HANDSET_CONNECTED_IDLE");
                if(appUserIsHeadsetPowerOn() && appChargerIsDisconnected())
                {
        ...
        case SFWD_OTA_MSG_SYNC_VOICE:            
            {
                DEBUG_LOG("SFWD_OTA_MSG_SYNC_DISABLE_VOICE");
                if(appUserIsHeadsetPowerOn() && appChargerIsDisconnected())
                {
                    if (appConfigIsLeft() && appHfpIsConnected() && (!appHfpIsCall())) 
                    {
...
static void appScoFwdEnterConnectedActiveReceive(void)
{
    ...
    //use to peer show led patter when sco active
    appUiHfpCallActive();
...
void appHfpSetMediaStarConfirmFlag(void)
{
    can_start_a2dp_stream = 1;
}

void appHfpClearMediaStarConfirmFlag(void)
{
    can_start_a2dp_stream = 0;
}

void appScoFwdHandleStartReceiveChain(void)
{
    ...
    appHfpSetMediaStarConfirmFlag();
}

void appScoFwdHandleStopReceiveChain(void)
{
    ...
    appHfpClearMediaStarConfirmFlag();
}
...
/*user define OTA control message*/
void appScoFwdUserPowerOff(void)
{
	SendOTAControlMessage(SFWD_OTA_MSG_USER_POWER_OFF);
...
{
	SendOTAControlMessage(SFWD_OTA_MSG_SYNC_LED_HANDSET_CONNECTED_IDLE);
   
}

void appScoFwdSyncLedLinkBack(void)
{
#ifdef LINK_BACK_LED_SYNC
	SendOTAControlMessage(SFWD_OTA_MSG_SYNC_LED_LINKBACK);
#endif
}

void appScoFwdSyncLedHandsetPairingSuccess(void)
{
	SendOTAControlMessage(SFWD_OTA_MSG_SYNC_LED_HANDSET_PAIRING_SUCCESS);   
}

void appScoFwdSyncVoice(void)
{
	SendOTAControlMessage(SFWD_OTA_MSG_SYNC_VOICE);   
}
