...
static uint8 BleDeviceName_L[] = "HEYDAY 05 L BLE";
static uint8 BleDeviceName_R[] = "HEYDAY 05 R BLE";
...
static void saveLocalName(const CL_DM_LOCAL_NAME_COMPLETE_T *name)
{
    ...
    if (name->status == hci_success)
    {
        ...
        if(appConfigIsLeft())
        {
            advMgr->localName = BleDeviceName_L;
        }
        else
        {
            advMgr->localName = BleDeviceName_R;
        }
#endif
    }
...
bool appAdvManagerHandleConnectionLibraryMessages(MessageId id, Message message, bool already_handled)
{
    switch (id)
    {
        ...
        case CL_DM_BLE_ADVERTISING_REPORT_IND:       
            appScanAdvertisingReportInd((const CL_DM_BLE_ADVERTISING_REPORT_IND_T *)message);    
			already_handled = 1;
        break;
	#endif
    }
...
#ifdef SINGLE_BDADDR
void appAdvStopAdvertising(void)
{
	ConnectionDmBleSetAdvertiseEnable(FALSE);
}

void appAdvStartAdvCurrentUseBdaddr(void)
{
	bdaddr current_addr;
	...
	if(appUiGetStoreAddr(&current_addr,PSKEY_CURRENT_USE_ADDR))
	{
		...
		parm.undirect_adv.adv_interval_min = 0x20;
		parm.undirect_adv.adv_interval_max = 0x40;
		parm.undirect_adv.filter_policy    = ble_filter_none;
		uint8 adv_data[8];
		DEBUG_LOG("start advertising bdaddr");
		adv_data[0] = 0x76;
		adv_data[1] = 0x85;/*special id */
	    adv_data[2] =  current_addr.lap & 0xFF;
	    adv_data[3] = (current_addr.lap >> 8) & 0xFF;
	    adv_data[4] = (current_addr.lap >> 16) & 0xFF;
	    adv_data[5] =  current_addr.uap;
	    adv_data[6] = (current_addr.nap >> 8) & 0xFF;
	    adv_data[7] = current_addr.nap & 0xFF;
	
		appGetAdvManager()->blockingOperation = APP_ADVMGR_ADVERT_SET_DATA_CFM; 
		ConnectionDmBleSetAdvertisingParamsReq(ble_adv_nonconn_ind, FALSE,	BLE_ADV_CHANNEL_ALL, &parm);
		ConnectionDmBleSetAdvertisingDataReq(8,adv_data);
