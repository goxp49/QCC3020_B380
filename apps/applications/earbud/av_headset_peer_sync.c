...
/*!@} */
#ifdef SINGLE_BDADDR
#define PEER_SYNC_MSG_STATE2_CHARGER_CONNECTED		(1 << 2)  
#endif
...
#ifdef SINGLE_BDADDR
#define PEER_SYNC_STATE_IS_CHARGER_CONNECTED(x) \
   ((PEER_SYNC_GET_STATE2(x) & PEER_SYNC_MSG_STATE2_CHARGER_CONNECTED) ? TRUE : FALSE)
#endif
/*!@} */
...
/*!@} */
#ifdef SINGLE_BDADDR
#define PEER_SYNC_STATE_SET_IS_CHARGER_CONNECTED(x) \
      ((x) ? PEER_SYNC_MSG_STATE2_CHARGER_CONNECTED : 0)
#endif
static uint16 appPeerSyncReadUint16(const uint8 *data)
{
...
static void appPeerSyncUpdatePeerInCase(bool peer_in_case)
{
    ...
    if (!current_peer_in_case_state && peer_in_case)
    {
        ...
        //add by liubing 2020_0103 for start ble advertsing*/
        #if 1
...
void appPeerSyncSend(bool response)
{
    ...
    if (appDeviceGetPeerBdAddr(&peer_addr))
    {
		...
		const uint8 state2 = (PEER_SYNC_STATE_SET_IS_HANDSET_PAIRED(!BdaddrIsZero(&paired_handset_addr))) +
							 ...
							 (PEER_SYNC_STATE_SET_IS_CHARGER_CONNECTED(appChargerIsConnected()));
...
static void appPeerSyncHandlePeerSigMsgChannelTxInd(const PEER_SIG_MSG_CHANNEL_RX_IND_T *ind)
{
	...
	#ifdef SINGLE_BDADDR		
	ps->peer_charger_connected	 = PEER_SYNC_STATE_IS_CHARGER_CONNECTED(ind->msg);
	#else
...
bool appPeerSyncIsChargerConnected(void)
{
    peerSyncTaskData* ps = appGetPeerSync();
    if (appPeerSyncIsComplete())
        return ps->peer_charger_connected;
    else
        return FALSE;
}
