...
#ifdef SINGLE_BDADDR
#define AVRCP_PEER_CMD_TDL_MSG  (0x42)

/*TDL message contain all PSKEY about PDL and attribut 100~ 110~ 139 140 141 142~*/
#define TDL_MESSAGE_SIZE_1 						(241)	/*index 0~3*/
#define TDL_MESSAGE_SIZE_2 						(38)	/*index 4~7*/
#define PACKEY_FLAG_OFFSET				        (240)


#define MAX_TDL_SIZE 							(8)


/*PSKEY whick keep paired record*/
#define PSKEY_TDL_ATTRIBUTE_BASE                (100)
#define ATTRIBUTE_BASE_PKESY_SIZE				(14)	/*bytes*/

#define PSKEY_PEER_TDL_ATTRIBUTE_BASE			(110)
#define PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE		(14)

#define PSKEY_SM_DIV_STATE                      (139)
#define SM_DIV_STATE_PSKEY_SIZE					(2)		/*bytes*/

#define PSKEY_SM_KEY_STATE_IR_ER                (140)
#define SM_KEY_STATE_IR_ER_PSKEY_SIZE			(32)

/* The Trusted Device List uses up to 9 PS Keys. 1 index and 8 for storing 
 * the link keys
 */
#define PSKEY_TRUSTED_DEVICE_LIST               (141)
#define TRUSTED_DEVICE_LIST_PSKEY_SIZE			(4)


#define PSKEY_TDL_BASE_INDEX					(142)
#define TDL_BASE_INDEX_PSKEY_SIZE				(32)


/*User define PSKEY use to keep peer device paired record*/
#define PSKEY_TDL_ATTRIBUTE_BACKUPS				(210)

#define PSKEY_PEER_TDL_ATTRIBUTE_BACKUPS		(220)

#define PSKEY_SM_DIV_STATE_BACKUPS              (239)
#define PSKEY_SM_KEY_STATE_IR_ER_BACKUPS        (240)

#define PSKEY_TRUSTED_DEVICE_LIST_BACKUPS       (241)
#define PSKEY_TDL_BASE_INDEX_BACKUPS			(242)	


static bool appPeerSigHandleTdlMessageInd(AV_AVRCP_VENDOR_PASSTHROUGH_IND_T *ind);
...
//add by liubing 2020_0103 for enable BLE adv for peerdisconnect 
...
{
    bool rc = FALSE;
	DEBUG_LOG("appPeerSigHandleAvAvrcpVendorPassthroughInd IND opid %x, payload size %x",ind->opid,ind->size_payload);
    switch (ind->opid)
    {
        case AVRCP_PEER_CMD_ADD_LINK_KEY:
        ...
        case AVRCP_PEER_CMD_CONNECT_HANDSET:
            ...
            break;
	#ifdef SINGLE_BDADDR
		case AVRCP_PEER_CMD_TDL_MSG:
			rc = appPeerSigHandleTdlMessageInd(ind);
			break;
	#endif
...
#ifdef SINGLE_BDADDR
		case AVRCP_PEER_CMD_TDL_MSG:
			DEBUG_LOG("vendor confirm message vendor opid AVRCP_PEER_CMD_TDL_MSG");
 			break;
#endif
...
#ifdef SINGLE_BDADDR
void appPeerSigTranferTdlMessage(void)
{
	uint8 message[TDL_MESSAGE_SIZE_1];
	uint16 index;
	uint16 data[16];
	uint16 d_offset=0;
	
	memset(message, 0, TDL_MESSAGE_SIZE_1); 
	
	if(!ConnectionTrustedDeviceListSize())
		return;
	DEBUG_LOG("appPeerSigTranferTdlMessage");
	for(index=0;index<appConfigMaxPairedDevices();index++)
	{
		memset(data, 0, 32);
		PsRetrieve(PSKEY_TDL_ATTRIBUTE_BASE+index,data, (ATTRIBUTE_BASE_PKESY_SIZE/2));
		memcpy(&message[d_offset], data, ATTRIBUTE_BASE_PKESY_SIZE);
		d_offset += ATTRIBUTE_BASE_PKESY_SIZE;
		
		PsRetrieve(PSKEY_PEER_TDL_ATTRIBUTE_BASE+index,data, (PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE/2));
		memcpy(&message[d_offset], data, PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE);
		d_offset += PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE;
		
		PsRetrieve(PSKEY_TDL_BASE_INDEX+index,data, (TDL_BASE_INDEX_PSKEY_SIZE/2));
		memcpy(&message[d_offset], data, TDL_BASE_INDEX_PSKEY_SIZE);
		d_offset += TDL_BASE_INDEX_PSKEY_SIZE;

		
	}
	message[PACKEY_FLAG_OFFSET] = 0x01; /*packet flag*/
	appPeerSigVendorPassthroughRequest(&appGetPairing()->task, AVRCP_PEER_CMD_TDL_MSG,
			   TDL_MESSAGE_SIZE_1, message);

	memset(message, 0, TDL_MESSAGE_SIZE_1);
	d_offset = 0;

	PsRetrieve(PSKEY_SM_DIV_STATE,data, (SM_DIV_STATE_PSKEY_SIZE/2));
	memcpy(&message[d_offset], data, SM_DIV_STATE_PSKEY_SIZE);
	d_offset += SM_DIV_STATE_PSKEY_SIZE;
	PsRetrieve(PSKEY_SM_KEY_STATE_IR_ER,data, (SM_KEY_STATE_IR_ER_PSKEY_SIZE/2));
	memcpy(&message[d_offset], data, SM_KEY_STATE_IR_ER_PSKEY_SIZE);
	d_offset += SM_KEY_STATE_IR_ER_PSKEY_SIZE;
	PsRetrieve(PSKEY_TRUSTED_DEVICE_LIST,data, (TRUSTED_DEVICE_LIST_PSKEY_SIZE/2));
	memcpy(&message[d_offset], data, TRUSTED_DEVICE_LIST_PSKEY_SIZE);
	d_offset += TRUSTED_DEVICE_LIST_PSKEY_SIZE;
	
	appPeerSigVendorPassthroughRequest(&appGetPairing()->task, AVRCP_PEER_CMD_TDL_MSG,
						   TDL_MESSAGE_SIZE_2, message);
...
bool appPeerSigHandleTdlMessageInd(AV_AVRCP_VENDOR_PASSTHROUGH_IND_T *ind)
{
	uint8 index;
	uint16 d_offset=0;
	uint16 data[16];

	DEBUG_LOG("receive tdl message payload size %x",ind->size_payload);
    
	if((appUserIsPageScanDisable() || appChargerIsConnected()) && (PsRetrieve(PSKEY_TDL_BASE_INDEX_BACKUPS, data, 16) == 16))
    {
        return TRUE;
    }   
    
	if(ind->size_payload == TDL_MESSAGE_SIZE_1)
	{
		if(ind->payload[PACKEY_FLAG_OFFSET] == 0x01)
		{
			d_offset = 0;
			for(index=0;index<appConfigMaxPairedDevices();index++)
			{
				memcpy(data, &ind->payload[d_offset], ATTRIBUTE_BASE_PKESY_SIZE);
				PsStore(PSKEY_TDL_ATTRIBUTE_BACKUPS+index,data,(ATTRIBUTE_BASE_PKESY_SIZE/2));
				d_offset += ATTRIBUTE_BASE_PKESY_SIZE;

				memcpy(data, &ind->payload[d_offset], PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE);
				PsStore(PSKEY_PEER_TDL_ATTRIBUTE_BACKUPS+index,data,(PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE/2));
				d_offset += PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE;
				
				memcpy(data, &ind->payload[d_offset], TDL_BASE_INDEX_PSKEY_SIZE);
				PsStore(PSKEY_TDL_BASE_INDEX_BACKUPS+index,data,(TDL_BASE_INDEX_PSKEY_SIZE/2));
				d_offset += TDL_BASE_INDEX_PSKEY_SIZE;
	...
	else if(ind->size_payload == TDL_MESSAGE_SIZE_2)	
	{

		memcpy(data, &ind->payload[d_offset], SM_DIV_STATE_PSKEY_SIZE);
		PsStore(PSKEY_SM_DIV_STATE_BACKUPS,data,(SM_DIV_STATE_PSKEY_SIZE/2));
		d_offset += SM_DIV_STATE_PSKEY_SIZE;

		memcpy(data, &ind->payload[d_offset], SM_KEY_STATE_IR_ER_PSKEY_SIZE);
		PsStore(PSKEY_SM_KEY_STATE_IR_ER_BACKUPS,data,(SM_KEY_STATE_IR_ER_PSKEY_SIZE/2));
		d_offset += SM_KEY_STATE_IR_ER_PSKEY_SIZE;
...
void appPeerSigExchangeTdlRecord(void)
{
	uint16 index;
	uint16 temp1[16];
	uint16 temp2[16];
    
	DEBUG_LOG("appPeerSigExchangeTdlRecord");
    
	for(index=0;index<appConfigMaxPairedDevices();index++)
	{
		memset(temp1,0,32);
		memset(temp2,0,32);
		PsRetrieve(PSKEY_TDL_ATTRIBUTE_BASE+index,temp1, (ATTRIBUTE_BASE_PKESY_SIZE/2));
		PsRetrieve(PSKEY_TDL_ATTRIBUTE_BACKUPS+index,temp2, (ATTRIBUTE_BASE_PKESY_SIZE/2));
		PsStore(PSKEY_TDL_ATTRIBUTE_BASE+index,temp2,(ATTRIBUTE_BASE_PKESY_SIZE/2));
		PsStore(PSKEY_TDL_ATTRIBUTE_BACKUPS+index,temp1, (ATTRIBUTE_BASE_PKESY_SIZE/2));

		PsRetrieve(PSKEY_PEER_TDL_ATTRIBUTE_BASE+index,temp1, (PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE/2));
		PsRetrieve(PSKEY_PEER_TDL_ATTRIBUTE_BACKUPS+index,temp2, (PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE/2));
		PsStore(PSKEY_PEER_TDL_ATTRIBUTE_BASE+index,temp2,(PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE/2));
		PsStore(PSKEY_PEER_TDL_ATTRIBUTE_BACKUPS+index,temp1, (PSKEY_PEER_TDL_ATTRIBUTE_BASE_SIZE/2));
		
		PsRetrieve(PSKEY_TDL_BASE_INDEX+index,temp1, (TDL_BASE_INDEX_PSKEY_SIZE/2));
		PsRetrieve(PSKEY_TDL_BASE_INDEX_BACKUPS+index,temp2, (TDL_BASE_INDEX_PSKEY_SIZE/2));
		PsStore(PSKEY_TDL_BASE_INDEX+index,temp2,(TDL_BASE_INDEX_PSKEY_SIZE/2));
		PsStore(PSKEY_TDL_BASE_INDEX_BACKUPS+index,temp1, (TDL_BASE_INDEX_PSKEY_SIZE/2));		
	}
	memset(temp1,0,32);
	memset(temp2,0,32);
	
	PsRetrieve(PSKEY_SM_DIV_STATE,temp1, (SM_DIV_STATE_PSKEY_SIZE/2));
	PsRetrieve(PSKEY_SM_DIV_STATE_BACKUPS,temp2, (SM_DIV_STATE_PSKEY_SIZE/2));
	PsStore(PSKEY_SM_DIV_STATE,temp2,(SM_DIV_STATE_PSKEY_SIZE/2));
    PsStore(PSKEY_SM_DIV_STATE_BACKUPS,temp1, (SM_DIV_STATE_PSKEY_SIZE/2));

	PsRetrieve(PSKEY_SM_KEY_STATE_IR_ER,temp1, (SM_KEY_STATE_IR_ER_PSKEY_SIZE/2));
	PsRetrieve(PSKEY_SM_KEY_STATE_IR_ER_BACKUPS,temp2, (SM_KEY_STATE_IR_ER_PSKEY_SIZE/2));
	PsStore(PSKEY_SM_KEY_STATE_IR_ER,temp2,(SM_KEY_STATE_IR_ER_PSKEY_SIZE/2));
    PsStore(PSKEY_SM_KEY_STATE_IR_ER_BACKUPS,temp1, (SM_KEY_STATE_IR_ER_PSKEY_SIZE/2));

	PsRetrieve(PSKEY_TRUSTED_DEVICE_LIST,temp1, (TRUSTED_DEVICE_LIST_PSKEY_SIZE/2));
	PsRetrieve(PSKEY_TRUSTED_DEVICE_LIST_BACKUPS,temp2, (TRUSTED_DEVICE_LIST_PSKEY_SIZE/2));
	PsStore(PSKEY_TRUSTED_DEVICE_LIST,temp2,(TRUSTED_DEVICE_LIST_PSKEY_SIZE/2));
    PsStore(PSKEY_TRUSTED_DEVICE_LIST_BACKUPS,temp1, (TRUSTED_DEVICE_LIST_PSKEY_SIZE/2));
...
void appPeerSigReinit(void)
{
	peerSigTaskData *peer_sig = appGetPeerSig();
	
	
	peer_sig->av_inst->avrcp.state = AVRCP_STATE_DISCONNECTED;
	peer_sig->av_inst->a2dp.state = A2DP_STATE_DISCONNECTED;
	peer_sig->lock = 0;
	peer_sig->link_loss_occurred = FALSE;
