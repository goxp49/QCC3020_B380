...
#define AUDIO_PSKEY_GAIN_NARROW	    (0x200380UL)
...
//static const uint16 NoiseWideParam[] = { 0x1009 ,0x0000 ,0x0001 ,0x0000 ,0x380C ,0x0006 ,0x0002 ,0xEEFF ,0x9600 ,0xE6FF ,0xCA00 ,0x003B ,0x0001 ,0xF214 ,0x82C0 ,0x003F ,0x0002 ,0xE715 ,0xE600 ,0x03F7 ,0xCED9 ,0x0043 ,0x0001 ,0x0000 ,0x005A ,0x004A ,0x0001 ,0x0000 ,0x0000 ,0x004D ,0x0001 ,0xF906 ,0x2200 ,0x0057 ,0x0001 ,0x0007 ,0xAE14 ,0x0061 ,0x0001 ,0xEC00 ,0x0000};
...
static void appSmDeletePairingAndReset(void)
{
    ...
    PsFlood();
#if 0 //#ifdef BM_POWER_ON 
	MessageSendLater(appGetUiTask(), APP_POWER_ON_TIME_OUT,0,D_SEC(2));
#else


    MessageSendLater(appGetUiTask(), APP_POWER_REBOOT,0,D_SEC(2));
...
//            appUiAvLinkLoss();
        ...
        else if (appDeviceIsHandset(&ind->bd_addr))
        {
            ...
            //Fixed a single address software boot error repeatedly into the link-loss
            if(appGetState() > APP_STATE_HANDSET_PAIRING)
            {
                appUiAvDisconnected();
                appUserSetLinkLossState(TRUE);
                appLedSetPattern(app_led_pattern_linkback_handset, LED_PRI_MED);
                
                //sync led status
                appScoFwdSyncLedLinkBack();
                //start ble scan for HUWEI
                appScanBleStartScanning();
                
                MessageSendLater(appGetUiTask(), APP_LINK_LOSS,0, appConfigLinkLossWaitBleScanTime());
                
//                appConnRulesSetEvent(appGetSmTask(), RULE_EVENT_HANDSET_LINK_LOSS);
            }
            else
            {
                appConnRulesResetEvent(RULE_EVENT_HANDSET_LINK_LOSS);
            }
...
//    appUserSetAudiokey(AUDIO_PSKEY_NOISE_WIDE,NoiseWideParam,(sizeof(NoiseWideParam)/sizeof(NoiseWideParam[0])));
...
static void appSmHandlePairingPeerPairConfirm(PAIRING_PEER_PAIR_CFM_T *cfm)
{
    ...
    switch (appGetState())
    {
        case APP_STATE_PEER_PAIRING:
        {
             ...
             * will try again */
            #ifdef PEER_PAIRING_TIMEOUT_EN
			 if(cfm->status == pairingSuccess)
             {
            	appSetState(APP_STATE_STARTUP);
             }
			 else if(cfm->status == pairingPeerTimeout)
			 {
				if(!appDeviceIsHandsetAnyProfileConnected() && !appPeerSyncIsPeerHandsetA2dpConnected() && !appPeerSyncIsPeerPairing())
				{
                    DEBUG_LOG("###pairingHandsetTimeout#####");
		...
		case APP_STATE_HANDSET_PAIRING:
             DEBUG_LOG("APP_STATE_HANDSET_PAIRING ");
		break; 
#endif
        default:
...
#ifdef LINK_LOSS
void appSmHandleConnectHandset(void)
{
	MessageCancelAll(appGetSmTask(), SM_INTERNAL_CONNECT_HANDSET);
	...
	if((!appDeviceIsHandsetAvrcpConnected()) || (!appDeviceIsHandsetA2dpConnected()))
	{
...
static void appSmHandleAvAvrcpConnectedInd(const AV_AVRCP_CONNECTED_IND_T *ind)
{
    ...
    if (appDeviceIsPeer(&ind->bd_addr))
    {
		...
		#ifdef SINGLE_BDADDR
			MessageSendLater(appGetUiTask(), APP_USER_TRANFER_PEER_TDL, 0, D_SEC(1));
		#endif
    }
}
...
static void appSmHandleInternalFactoryReset(void)
{
    ...
    else
    {
        ...
        MessageSendLater(appGetUiTask(),APP_RESET_LED_STOP, 0, D_SEC(2));
    }
#endif
}
...
static void appSmHandleNoDfu(void)
{
    ...
    if (appGetState() != APP_STATE_IN_CASE_DFU)
    {
		...
		{
			uint16 data[1] = {0};			
			PsRetrieve(PSKEY_CHG_STATE_KEY, data, 1);	
			
			if(data[0] == 0x1111)
			{		
				PsStore(PSKEY_CHG_STATE_KEY, NULL, 0);			
                appUserEnablePowerOff();
				appUserPowerOn();
			...
			{
			    //clear power key press
			    MessageSendLater(appGetUiTask(), APP_POWER_ON_PRESS_CHECK, 0, 100);
                
				/*if power on operation failed ,need to go back compelte off state*/
				MessageSendLater(appGetUiTask(), APP_POWER_ON_TIME_OUT, 0, D_SEC(10));
			}
		}
...
#ifdef LINK_LOSS
static void appSmHandleInternalLinkLossReconnect(void)
{
    DEBUG_LOG("appSmHandleInternalLinkLossReconnect");
    smTaskData *sm = appGetSm();
	uint8 count = sm->linkloss_reconnect_count;
	
	MessageCancelAll(&sm->task, SM_INTERNAL_CONNECT_HANDSET);
	MessageCancelAll(&sm->task, SM_INTERNAL_TIMEOUT_LINK_LOSS_RECONNECT);

	
	if (appSmIsOutOfCase() && appConfigLinkLossReconnectInterval())
	{
        if (appDeviceIsHandsetA2dpConnected() && appDeviceIsHandsetAvrcpConnected() && appDeviceIsHandsetHfpConnected())
        {
			sm->linkloss_reconnect_count = 0;
			appConnRulesResetEvent(RULE_EVENT_HANDSET_LINK_LOSS);
        }
		else if(count < appConfigLinkLossReconnectTimes())
		{
			sm->linkloss_reconnect_count = count + 1;
			MessageSend(&sm->task,SM_INTERNAL_CONNECT_HANDSET,NULL);
			MessageSendLater(&sm->task, SM_INTERNAL_TIMEOUT_LINK_LOSS_RECONNECT, 
								NULL, appConfigLinkLossReconnectInterval());
		}
		else
		{
			sm->linkloss_reconnect_count = 0;
			appConnRulesResetEvent(RULE_EVENT_HANDSET_LINK_LOSS);
			{
				DEBUG_LOG("link loss enter pairing mode");
				appUserEnterPairingMode();
			}
		}
	}
...
void appSmHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case CONN_RULES_REJECT_HANDSET_CONNECT:
            ...
            break;
	#ifdef LINK_LOSS
		case CONN_RULES_RECONNECT_HANDSET:
			appConnRulesSetRuleComplete(CONN_RULES_RECONNECT_HANDSET);
			MessageSendLater(appGetSmTask(), SM_INTERNAL_TIMEOUT_LINK_LOSS_RECONNECT, 
								NULL, appConfigLinkLossReconnectInterval());
			break;

		case SM_INTERNAL_TIMEOUT_LINK_LOSS_RECONNECT:
			appSmHandleInternalLinkLossReconnect();
			break;
    #endif
        ...
        case SM_INTERNAL_BREDR_CONNECTED:
            ...
            break;
#ifdef LINK_LOSS
		case SM_INTERNAL_CONNECT_HANDSET:
			appSmHandleConnectHandset();
			break;
#endif
...
#ifdef SINGLE_BDADDR
void appSmDisconnectAllLink(void)
{
	appSmInitiateLinkDisconnection(SM_DISCONNECT_ALL, 200,
	                           POST_DISCONNECT_ACTION_NONE);
}
#endif
