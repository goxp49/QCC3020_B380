...
{
    DEBUG_LOG("appA2dpStreamingInactiveUI");

    if (appA2dpIsSinkCodec(theInst))
    {
        DEBUG_LOG("appA2dpStreamingInactiveUi - pass");
...
static void appA2dpEnterConnectedSignalling(avInstanceTaskData *theInst)
{
	...
	#ifdef SINGLE_BDADDR	
		MessageCancelAll(appGetUiTask(), APP_USER_RECONNECT_HANDSET);
		if(appPeerSyncIsComplete())
		{
			if(appConfigIsLeft())
			{
				MessageSendLater(appGetUiTask(), APP_USER_TRANFER_PEER_TDL, 0, 200);
			}
			else
			{
				MessageSendLater(appGetUiTask(), APP_USER_TRANFER_PEER_TDL, 0, 300);
			}
		}
		else
			MessageSendLater(appGetUiTask(), APP_USER_TRANFER_PEER_TDL, 0, 500);
	#endif
    }
    else
    {
    ...
    #ifdef SINGLE_BDADDR
		appScanBleStopScanning();
        //add by liubing 2020_0103 for disable BLE adv when peer connected
        #if 1
...
#ifdef BM_POWER_ON
    /* Reject incomming connection is we are in off state*/
    if(!appUserIsHeadsetPowerOn() || (appUserIsResetState() == TRUE) || (appGetUi()->usb_attached == TRUE))
    {
        DEBUG_LOG("#######Avrcp Reject it#######");
...
//                appUiAvLinkLoss();
            ...
            else if (ind->status == a2dp_success)
            {
				...
				if (!appDeviceIsPeer(&theInst->bd_addr))
				{
					...
					{
						DEBUG_LOG("disconnected enter pairing mode");
...
#else
    //Imitate 6.42 and fix unexpected panic problem
    MessageCancelAll(&theInst->av_task, AV_INTERNAL_A2DP_INST_SYNC_RES);
#endif
...
#ifdef SINGLE_BDADDR
void appA2dpDestoryAvInstance(void)
{
    ...
    {
    	if(theAv->av_inst[instance])
    	{
    		appAvInstanceDestroy(theAv->av_inst[instance]);
    	}
		MessageCancelAll(&theAv->av_inst[instance]->av_task, AV_INTERNAL_AVRCP_DISCONNECT_LATER_REQ);
    }	
