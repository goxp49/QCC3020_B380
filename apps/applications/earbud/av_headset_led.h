...
typedef struct
{
    ...
    unsigned int data:12;	/*!< LED pins controlled by the code */
#else
...
#define LED_PATTERN_DIM_ON  (0x0008)         	/*!< Marker for dim led */
#define LED_PATTERN_DIM_OFF (0x0009)         	/*!< Marker for dim led */
...
#define LED_DIM_ON(pio,duty)	{LED_PATTERN_DIM_ON, (pio) << 8 | (duty)}			/*!< Led dim pattern*/
#define LED_DIM_OFF(pio,duty)	{LED_PATTERN_DIM_OFF, (pio) << 8 | (duty)}			/*!< Led dim pattern*/
