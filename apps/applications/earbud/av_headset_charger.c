...
static void appChargerCheck(void)
{
    ...
    if (is_connected != theCharger->is_connected)
    {
		...
		#ifdef SINGLE_BDADDR
			if(appGetUi()->usb_attached)
			{
				MessageSendLater(appGetUiTask(),APP_POWER_REBOOT,0,50);
			}
			else
			{
				MessageSendLater(appGetUiTask(),APP_POWER_REBOOT,0,1500);
			}
		#else
			MessageSendLater(appGetUiTask(),APP_POWER_REBOOT,0,50);
		#endif
...
//				appUserSetHeadsetPowerState(FALSE);
				...
				uint16 data = 0x1111;
				if(PsStore(PSKEY_CHG_STATE_KEY, &data, 1) != 1)
				{
					Panic();
				}															
//				appUserSetHeadsetPowerState(FALSE);
                ...
                //Stop peer pairing if need
                if(appGetState() == APP_STATE_PEER_PAIRING)
                {                                        
                    DEBUG_LOG("appPairingPeerPairCancel !!");
    ...
    if (is_connected == CHARGER_CONNECTED_NO_ERROR)
    {
        //stop charging led while reset
        if(!appUserIsResetState())
        {
...
static void appChargerHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        break;
#ifdef POWER_MANAGER
		case CHARGER_COMPLETE_POWER_OFF:
