...
/*! User define funcion*/
/*! use to work when device in link loss state, control auto reconnect to handset*/
#define LINK_LOSS

/*! when use button to control power on off, need to add this module*/
#define BM_POWER_ON

/*! if we have dim led display user interface */
#define DIM_LEDx

/*! use to improve power on reconnect fuction*/
#define POWER_ON_RECONNECT

/*! manager the device power control, such to low battery warning and low bat power off*/
#define POWER_MANAGER

/*! peer device use save addr when connect to handset, change addr when handover happended*/
#define SINGLE_BDADDR

/*! enable button multi click detect, such as double click, triple click*/
#define BM_MUL_CLICK

/*! enable to exit peer pairing when pairing time out*/
#define PEER_PAIRING_TIMEOUT_EN

/*! when incomming disconnect request happened */
#define ENTER_PAIRING_HANDSET_DISCONNECT

#define BM_ENTER_HANDSET_PAIRING

#define ENABLE_EQ_CHAIN
...
#ifdef LINK_LOSS
/*完整的回连时间还要加上最后一次回连附带的另外两次连接的时间*req->num_retries*/
#define appConfigLinkLossReconnectTimeout()		(D_SEC(120) - appConfigLinkLossReconnectInterval() - appConfigLinkLossWaitBleScanTime())
#define appConfigLinkLossReconnectInterval()	(D_SEC(10))
#define appConfigLinkLossReconnectTimes()		(appConfigLinkLossReconnectTimeout()/appConfigLinkLossReconnectInterval())
#define appConfigLinkLossWaitBleScanTime()      (D_SEC(8))
	...
	#define appConfigIsRealLeft()	 	(appGetInit()->appInitIsRealLeft)
	#define appConfigIsRealRight()		(appConfigIsRealLeft() ^ 1)
#endif

#define LINK_BACK_LED_SYNCx

#define LOCAL_NAME_CONFIGURATION_UNSUPPORTEDx

/**/

typedef enum
{
     PSKEY_SOFTWARE_VERSION=200,
     PSKEY_ENCRYPT_LEVEL=201,
     PSKEY_LICENSE_KEY=202,
...
#define POWER_ON_LINKBACK_TIMEOUT                       D_SEC(15)
...
#define appConfigMaxHandsetPairingAttempts() (7)
...
/*! Timeout for entering the case after selecting DFU from the user
    interface */
#define appConfigDfuTimeoutToPlaceInCaseMs()        (D_SEC(300))
...
#define appConfigPeerPairingTimeout()       (15)
/*! Timeout in seconds for user initiated handset pairing */
#define appConfigHandsetPairingTimeout()    (300)
...
#define appConfigAutoPeerPairingTimeout()       (15)
...
#define appConfigBatteryVoltageLow()        (3500)
...
#define appConfigBatteryReadPeriodMs() D_SEC(10)
...
#define appConfigAvrcpConnectDelayAfterRemoteA2dpConnectMs() D_SEC(2)
...
#define appConfigPromptNoRepeatDelay() D_SEC(0)//D_SEC(5)
