...
static void kymera_msg_handler(Task task, MessageId id, Message msg)
{
    ...
    switch (id)
    {
        ...
        {
            DEBUG_LOG("KYMERA_INTERNAL_SCO_SLAVE_START");

            if(scoSlaveStartFlag == TRUE)
            {
                DEBUG_LOG("ignore!!!!");
                return;
            }
 ...
 /* If we are not idle (a pre-requisite) and this message can be delayed,
        ...
        && appKymeraGetState() != KYMERA_STATE_IDLE)
    {
        ...
        return;
            ...
            {
                DEBUG_LOG("send!");
                scoSlaveStartFlag = TRUE;
                MessageCancelAll(&theKymera->task, id);
