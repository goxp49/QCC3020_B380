...
uint16 can_start_a2dp_stream=0;

extern uint16 *CanStartA2DPstream(void);

uint16 *CanStartA2DPstream(void)
{
	return &can_start_a2dp_stream;
}
#endif
...
#ifdef POWER_MANAGER
	MessageSendLater(appGetUiTask(),APP_IND_BAT_LEVEL_TO_PHONE,0,D_SEC(2));
#endif
...
//                 appUiHfpLinkLoss();
            ...
            else
            {
                ...
                if(appUserIsResetState() == FALSE)
                {
                    DEBUG_LOG("appUserIsResetState(%d)-----1", appUserIsResetState());
        ...
        case HFP_STATE_DISCONNECTED:
        {
            ...
            if(appUserIsResetState() == FALSE)
            {
                DEBUG_LOG("appUserIsResetState(%d)-----2", appUserIsResetState());
                appUiHfpDisconnected();
            }
...
//    bool ring_fwd = appConfigScoForwardingEnabled() && appDeviceIsPeerRingForwardSupported() && appDeviceIsPeerConnected();
    bool ring_fwd = appConfigScoForwardingEnabled() && appDeviceIsPeerConnected();
