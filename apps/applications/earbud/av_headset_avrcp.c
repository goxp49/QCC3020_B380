...
#if 1
    if (!appUserIsHeadsetPowerOn()||(appGetPower()->user_initiated_shutdown == TRUE)||(appUserIsResetState() == TRUE)||(appGetUi()->usb_attached == TRUE))
    {
...
{
    assert(theInst->avrcp.avrcp == ind->avrcp);
    DEBUG_LOGF("appAvrcpHandleAvrcpPassthroughIndication(%p) %d, opid %d", (void *)theInst, ind->state, ind->opid);
        ...
        {        
            ...
            switch (ind->opid)
            {
                ...
                case opid_vendor_unique:
                {
                    ...
                    {
                    	DEBUG_LOG("opid_vendor_unique, data size %x", ind->size_op_data);
                        ...
                        if (ind->size_op_data >= 5)
                        {
                            ...
                            if (company_id == appConfigIeeeCompanyId())
                            {
                                ...
                                if((opid == 0x41) && (ind->size_op_data == 12) && (ind->op_data[11] == 2))
                                {
                                    appScoFwdHandleStopReceiveChain();
                                    AvrcpPassthroughResponse(ind->avrcp, avctp_response_accepted);
                                    return;
                                }
                                else if((opid == 0x41) && (ind->size_op_data == 12) && (ind->op_data[11] == 1))
                                {
